...
| [KIT DSN dossiers](https://www.dsn.kastel.kit.edu/bitcoin/snapshots/) | KIT DSN group (Karlsruhe Institute of Technology) | Daily research crawl dossier (about 6 MB JSON) with per-node whois/ASN annotations; public keys are hashed (no raw IPs). Full-dossier clearnet count matches the [live churn plot](https://www.dsn.kastel.kit.edu/bitcoin/) unique-IP scale (~9–10k); `lastConnect` windows are stricter subsets. Backfilled daily from 2026-05-10 (same start as btcnodes). | daily + historic backfill |
